function [vp0,vp1,vp2,vp3,vp4,vp5,vp6] = gsw_specvol_p_parts(SA,CT)
...
% 
% USAGE:  
%  [vp0,vp1,vp2,vp3,vp4,vp5,vp6] = gsw_specvol_p_parts(SA,CT)
% 
% DESCRIPTION:
...
if ~(nargin == 2)
   error('gsw_specvol_p_parts:  Requires two inputs')
end
...
if (ms ~= mt | ns ~= nt )
   error('gsw_specvol_p_parts: SA and CT do not have the same dimensions')
end
...
vp0 = v000 + xs.*(v100 + xs.*(v200 + xs.*(v300 + xs.*(v400 + xs.*(v500 + xs.*v600))))) ...
...
vp1 =  v001 + xs.*(v101 + xs.*(v201 + xs.*(v301 + xs.*(v401 + xs.*v501)))) ...
...
vp2 = v002 + xs.*(v102 + xs.*(v202 + xs.*(v302 + xs.*v402))) ...
   + ys.*(v012 + xs.*(v112 + xs.*(v212 + xs.*v312)) ...
   + ys.*(v022 + xs.*(v122 + xs.*v222) ...
   + ys.*(v032 + xs.*v132 ...
   + ys.*v042)));

vp3 = v003 + xs.*(v103 + xs.*v203) ...
   + ys.*(v013 + xs.*v113 ...
   + ys.*v023);
...
% Note that,
% specvol = vp0 + z.*(vp1 + z.*(vp2 + z.*(vp3 + z.*(vp4 + z.*(vp5 + z.*vp6)))));
